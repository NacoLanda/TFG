...
==========================
Parsea los HTML descargados de FBref con estadísticas individuales de
jugadores de LaLiga 2025-26 y genera un Excel con dos hojas:
"Jugadores" (jugadores de campo) y "Porteros".

Fuentes HTML (descargadas manualmente de fbref.com):
  - Jugadores_Stats.html       → estadísticas estándar (goles, asistencias, etc.)
  - Jugadores_Porteros.html    → estadísticas de porteros
  - Jugadores_Tiros.html       → tiros y xG
  - Jugadores_Tiempo.html      → tiempo de juego y rotación
  - Jugadores_Miscelaneo.html  → faltas, fueras de juego, intercepciones

Flujo:
  1. Parsea cada HTML buscando la tabla por su id.
  2. Alinea y une las tablas por (jugador, equipo, año_nacimiento).
  3. Limpia y traduce nombres de columnas al español.
  4. Convierte valores numéricos (el HTML usa comas como separador de miles).
  5. Escribe el Excel con formato y anchos automáticos.

Salida: Datos/Jugadores FBref.xlsx  (2 hojas: Jugadores y Porteros)

Uso: python3 "Código Jugadores FBref.py"
"""
...
# ══════════════════════════════════════════════════════════════════
# CONFIGURACIÓN — rutas y archivos de entrada
# ══════════════════════════════════════════════════════════════════
DATA_DIR  = "/Users/NacoLG/Documentos/UFV 4/TFG/Ingeniería del Dato/Descargas FBref"
OUTPUT    = "/Users/NacoLG/Documentos/UFV 4/TFG/Ingeniería del Dato/Datos/Jugadores FBref.xlsx"

FILES = [
    ("Stats",      "Jugadores_Stats.html",      "stats_standard"),
    ("Porteros",   "Jugadores_Porteros.html",   "stats_keeper"),
    ("Tiros",      "Jugadores_Tiros.html",       "stats_shooting"),
    ("Tiempo",     "Jugadores_Tiempo.html",      "stats_playing_time"),
    ("Miscelaneo", "Jugadores_Miscelaneo.html",  "stats_misc"),
]
...
# Funciones de limpieza aplicadas antes de unir las tablas.
# ══════════════════════════════════════════════════════════════════
def clean_nation(value: str) -> str:
    """Extrae el código de país en mayúsculas del campo de nacionalidad de FBref."""
    return " ".join(re.findall(r'[A-Z]+', value))

def clean_position(value: str) -> str:
    """Traduce los códigos de posición de FBref (GK, DF, MF, FW) al español."""
...
def to_number(value: str):
    ...
    if value == "" or value is None:
        return None
    try:
        # El HTML usa coma como separador de miles (ej: "1,474"), la eliminamos antes de parsear
        return float(value.replace(",", ""))
    except (ValueError, TypeError):
        return value

def apply_transformations(df: pd.DataFrame) -> pd.DataFrame:
    """Aplica clean_nation y clean_position si esas columnas están presentes."""
...
def apply_numeric_conversion(df: pd.DataFrame) -> pd.DataFrame:
    """Convierte a numérico todas las columnas que no sean de texto (jugador, equipo…)."""
...
# FBref genera tablas con filas de cabecera repetidas dentro del
# tbody (class="thead"). Las filtramos para quedarnos solo con datos.
# ══════════════════════════════════════════════════════════════════
def parse_table(filename: str, table_id: str) -> pd.DataFrame:
    """
    Parsea una tabla de FBref desde un archivo HTML local.

    Args:
        filename: Nombre del archivo HTML dentro de DATA_DIR.
        table_id: Atributo id del elemento <table> en el HTML.

    Returns:
        DataFrame con una fila por jugador y una columna por stat.
    """
...
# ══════════════════════════════════════════════════════════════════
# CONSTRUCCIÓN Y UNIÓN DE DATAFRAMES
# Unimos las 5 tablas por (jugador, equipo, año_nacimiento).
# Los porteros se procesan por separado porque sus estadísticas
# son distintas a las de los jugadores de campo.
# ══════════════════════════════════════════════════════════════════
def build_dataframes():
    """
    Parsea todos los HTML y construye los DataFrames de jugadores y porteros.

    Para jugadores de campo: une Stats, Tiros, Tiempo y Misceláneo.
    Para porteros: usa únicamente la tabla de porteros (sus stats son distintas).

    Returns:
        df_field: DataFrame de jugadores de campo listo para exportar.
        df_gk:    DataFrame de porteros listo para exportar.
    """
...
# Traducimos los nombres de los campos de porcentaje al español renombrado
# para identificarlos correctamente al aplicar el formato numérico en Excel.
...
def write_sheet(wb: Workbook, df: pd.DataFrame, sheet_name: str):
    """
    Escribe un DataFrame en una hoja del excel con formato profesional.

    Aplica colores de cabecera, filas alternadas, bordes y ajuste
    automático de ancho de columnas. Fija la primera fila y columna.
    """
    ...
    for col_idx, col_name in enumerate(df.columns, start=1):
        if col_name in TEXT_COLS:
            ...
            # Número con símbolo % — el valor ya es el número (ej: 65.8 = 65.8%)
